...
**Install configs (appliance mode)**: When `install_method == 'appliance'`, the `bastion_configure` role renders `install-config.yaml` and `agent-config.yaml` to `/home/cloud-user/appliance-install-configs/` as a starting point for the user. The agent-config includes per-host NMState static networking: trunk0 carries the management network (native/untagged, static IP from `ove_node_mgmt_ips`), trunk1 carries a VLAN sub-interface for the storage network (VLAN ID from `storage_vlan_id`, IP derived from mgmt IP host octet into `storage_network_cidr`). For libvirt, MAC addresses are pre-populated from the deterministic `mac_prefix` scheme; for OpenStack, they require manual update after node creation. Optional `mgmt_mtu` and `storage_mtu` variables control interface MTU settings and are omitted from the NMState config when unset.
